...
class Block(collections.namedtuple('Block', ['scope', 'unit_fn', 'args'])):
    '''
    使用collections.namedtuple设计ResNet基本的Block模块组的named tuple只包含数据结构,
    包含具体方法需要传入三个参数[scope,unit_fn,args]
...
def conv2d_same(inputs, num_outputs, kernel_size, stride, scope=None):
    ...
    if stride == 1:
        return slim.conv2d(
            inputs, num_outputs, kernel_size, stride=1, padding='SAME', scope=scope)
    else:
        ...
        return slim.conv2d(
            inputs, num_outputs, kernel_size, stride=stride, padding='VALID', scope=scope)
    ...
    with tf.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
        ...
        if depth == depth_in:
            ...
            shortcut = slim.conv2d(
                preact, depth, [1, 1], stride=stride,
...
def resnet_v2(inputs,
    ...
    Args:
        ...
        reuse: whether or not the network and its variables should be reused. To be
            le to reuse 'scope' must be given.
        scope: Optional variable_scope.
...
def resnet_v2_101(
    ...
    blocks = [
        ...
        Block('block4', bottleneck, [(2048, 512, 1)] * 3)]
    return resnet_v2(
        inputs, blocks, num_classes, global_pool,
        include_root_block=True, reuse=reuse, scope=scope)
...
def prepare_data_one(input_dir, output_dir):
    '''
    Only get a list of the training(or validation or testing) file paths

    Args:
        input_dir: The dir of the images.
        output_dir: The dir of the labels.

    Returns:
        input_names: list of the images file paths.
        output_names: list of the labels file paths.
    '''
    input_names = []
    output_names = []
    ...
    for file in os.listdir(input_dir):
        ...
        input_names.append(input_dir + "/" + file)
...
def load_image(path):
    ...
    # -1 = IMREAD_UNCHANGED; opencv惯用BGR 此处会影响对label的one-hot-it
    # image = cv2.imread(path, -1)
    cv2.resize(image, [224, 224, None])
    return image
    ...
    for i in range(num_batches + num_steps_burn_in):
        ...
        if i >= num_steps_burn_in:
            if not i % 10:
                ...
                # summary = session.run(merged, feed_dict=inputs)
...
# Main
imgs_dir = r"C:/WorkSpace/Data/train"
labels_dir = r"C:/WorkSpace/Data/train_labels"
log_dir = r'C:/WorkSpace/myTF/ISPRS_data_3/log'
height, width = 224, 224
classes_num = 2

inputs = tf.placeholder(tf.float32, shape=[None, None, None, 3])
output = tf.placeholder(tf.float32, shape=[None, None, None, classes_num])
# with slim.arg_scope(resnet_arg_scope(is_training=False)):
#     net, end_points = resnet_v2_152(inputs, 1000)
with slim.arg_scope(resnet_arg_scope(is_training=True)):
    network, end_points = resnet_v2_50(inputs, classes_num)


# Compute your (unweighted) softmax cross entropy loss
with tf.name_scope('cross_entropy'):
    loss = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits_v2(logits=network, labels=output))
...
train_input_names, train_output_names = prepare_data_one(imgs_dir, labels_dir)
...
for epoch in range(0, EPOCHS):
    ...
    num_iters = int(np.floor(len(id_list) / batch_size))  # 数据全部训练一周期需要的次数
    st = time.time()
    for i in range(num_iters):
        ...
        cnt = cnt + batch_size
        if cnt % 20 == 0:
            string_print = "Epoch = %d Count = %d Current_Loss = %.2f Time = %.2f" \
                % (epoch, cnt, cur_loss, time.time() - st)
            print(string_print)
            st = time.time()
            # collect summary and write them into file
            summary = sess.run(merged, feed_dict={
                input: input_image_batch,
                output: output_image_batch})
            train_writer.add_summary(summary, cnt)

# time_tensorflow_run(sess, network, "Forward")
