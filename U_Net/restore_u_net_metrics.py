...
new_saver = tf.train.import_meta_graph(
    r'/home/leilei/Ceshi/Model_fuse/model_X/model.ckpt-40000.meta')
# import weights bias
new_saver.restore(
    sess, save_path=r'/home/leilei/Ceshi/Model_fuse/model_X/model.ckpt-40000')
...
for i in range(len(names)):
    ...
    predict = np.array(
        sess.run(pre_2, feed_dict={
            image: imgs,
            is_training: False
        }))[0]
    # p = predict.astype(np.dtype.float64)
...
for k in range(len(names)):
    ...
    print(names[k] + ' ' + str(precision) + ' ' + str(recall) + ' ' +
          str(mean_iou))
