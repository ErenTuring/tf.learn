...
def Loss(logits,labels):
    ...
    segment_loss=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=labels,logits=logits))
    return segment_loss
...
def mean_IOU(pred,labels,num_classes):
    ...
    Confus_mat=tf.metrics.mean_iou(labels=labels,predictions=pred,num_classes=num_classes)[1]#[0] => mean_iou
    
    recall=tf.div(Confus_mat[1,1],tf.add(Confus_mat[1,1],Confus_mat[1,0]))
    precision=tf.div(Confus_mat[1,1],tf.add(Confus_mat[1,1],Confus_mat[0,1]))
    mean_iou=tf.div(Confus_mat[1,1],tf.add_n([Confus_mat[1,1],Confus_mat[1,0],Confus_mat[0,1]]))
...
def main():
    dataset_path=r'/home/leilei/ISPRS_data_3'
    train_list_path=r'/home/leilei/ISPRS_data_3/train.txt'
    ...
    ''' batch_data params '''
    dataset_train_param=batch_Data.Data(dataset_path,train_list_path,class_number)
    dataset_mean_vector=dataset_train_param.meanvector
    dataset_valid_param=batch_Data.Data(dataset_path,valid_list_path,class_number)
    
    image = tf.placeholder(tf.float32, shape = [batch_size,img_size,img_size,3],name='image')
    label = tf.placeholder(tf.int32, shape = [batch_size,img_size,img_size,class_number],name='label')
    ...
    with tf.name_scope('model'):
        ...
        tf.add_to_collection('pred',fcn1.pred)
        
    with tf.name_scope('loss'):
        loss=Loss_func(Loss(fcn1.new_score_1,label),Loss(fcn1.new_score_2,label),L2_loss(5e-4))
    
    global_step=tf.Variable(tf.constant(0))
    train_op=train(loss,learning_rate,10000,0.1,global_step)
    
    sess=tf.Session()
    accuracy=Accuracy(pred=fcn1.pred,labels=label)
...
#        print(sess.run(global_step))
        train_imgs,train_labs=dataset_train_param.next_batch(batch_size=batch_size,flag='train')
        ...
        if step%1000==0:
            valid_imgs,valid_labs=dataset_valid_param.next_batch(batch_size=batch_size,flag='valid')
            feed_dict={image:valid_imgs,label:valid_labs,is_training:False}
            accu=round(sess.run(accuracy,feed_dict=feed_dict),2)
            print('Accuracy: %.2f'%accu)
            saver.save(sess,save_path=r'/home/leilei/Ceshi/Model_fuse/model/model.ckpt',global_step=step)
    sess.close()
