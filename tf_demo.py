...
# @author: 陈水平
# @date: 2017-02-09
# @description: implement a softmax regression model
#               upon MNIST handwritten digits
...
# and we transform them into numpy ndarray
# objects by the following two utility functions
...
train_x_data = train_x_data.reshape(train_x_data.shape[0], -1).astype(
    np.float32)
...
# Of course you can also use the utility function
# to read in MNIST provided by tensorflow
...
if eval_sklearn:
    ...
    np.savetxt(
        'coef_softmax_sklearn.txt', reg.coef_,
        fmt='%.6f')  # Save coefficients to a text file
    test_y_predict = reg.predict(test_x_minmax)
    print("Accuracy of test set: %f"
          % accuracy_score(test_y_data, test_y_predict))
...
if eval_tensorflow:
        ...
        for step in range(1000):
            if step % 10 == 0:
                ...
                print("Stochastic Gradient ",
                      "Descent processing step %d accuracy=%.2f" % (step, acc))
            else:
    ...
    np.savetxt(
        'coef_softmax_tf.txt', np.transpose(sess.run(W)),
        fmt='%.6f')  # Save coefficients to a text file
    print("Accuracy of test set: %f" % sess.run(
        accuracy, feed_dict={
            x: test_x_minmax,
            y_: test_y_data_trans
        }))
# PC_name: DESKTOP-L9R8H0A
