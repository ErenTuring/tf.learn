...
'''
指数衰减学习率
函数定义exponential_decay(learning_rate, global_step, decay_steps,
...
def get_weight(shape, llamada):
    ...
    if llamada is not None:
        tf.add_to_collection('losses',
                             tflayers.l2_regularizer(llamada)(weights))
...
def train(mnist):
    ...
    y_ = tf.placeholder(
        tf.float32, shape=(None, OUTPUT_NODE), name='y_input')  # 标签值
    ...
    '''
    var_aver = tf.train.ExponentialMovingAverage(MOVING_AVERAGE_DECAY,
                                                 global_step)  # 创建滑动平均对象
    variable_averages_op = var_aver.apply(
        tf.trainable_variables())  # 对所有需要训练的变量应用滑动平均
        ...
        for i in range(TRAINING_STEPS):
            ...
            if i % 1000 == 0:
                ...
                validate_acc = sess.run(
                    accuracy, feed_dict=validate_feed)  # 验证集精度
                print(
...
def main(argv=None):
    ...
    print(mnist.validation.images.shape,
          mnist.validation.labels.shape)  # validation sets
    train(mnist)
