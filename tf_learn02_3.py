...
def inference(input_tensor, avg_class, weights1, biases1, weights2, biases2):
    ...
    if avg_class == None:
        ...
        layer_out = tf.matmul(layer1, weights2) + biases2
    else:
        # 首先需要使用avg_class.average函数计算变量的滑动平均值，然后再计算相应的神经网络前向传播结果
        layer1 = tf.nn.relu(
            tf.matmul(input_tensor, avg_class.average(weights1)) + avg_class.average(biases1))
        layer_out = tf.matmul(layer1, avg_class.average(weights2)) + avg_class.average(biases2)
...
def train(mnist):
    ...
    y_ = tf.placeholder(tf.float32, shape=(None, OUTPUT_NODE), name='y_input') # 标签值
    ...
    '''
    使用滑动平均模型
        1.初始化滑动平均的函数类，加入训练轮数的变量可以加快训练早期变量的更新速度
        2.对神经网络里所有可训练参数（列表）应用滑动平均模型，每次进行这个操作，列表里的元素都会得到更新
        3.计算使用了滑动平均的网络前向传播结果，滑动是维护影子变量来记录其滑动平均值,需要使用时要明确调用average函数
    '''
    var_aver = tf.train.ExponentialMovingAverage(MOVING_AVERAGE_DECAY, global_step) # 创建滑动平均对象
    variable_averages_op = var_aver.apply(tf.trainable_variables()) # 对所有需要训练的变量应用滑动平均
                                                            # tf.trainable_variables 返回的是需要训练的变量列表
                                                            # tf.all_variables 返回的是所有变量的列表
    average_y = inference(x, var_aver, weights1, biaes1, weights2, biaes2)

    '''
    定义loss
        当只有一个标准答案的时候，使用 sprase_softmax_cross_entropy_with_logits 计算损失，可以加速计算
    ...
    # get_collection返回一个列表，列表是'losses'这个集合的所有元素(加起来就得到了所有的损失)
        ...
        for i in range(TRAINING_STEPS):
            ...
            if i % 1000 == 0:
                print("After %d training step(s), loss on training batch is %g." 
                        % (step, loss_value))
                validate_acc = sess.run(accuracy, feed_dict=validate_feed) # 验证集精度
                print("After %d training step(s),validation accuracy using average model is %g " 
                        % (step, validate_acc))
                test_acc = sess.run(accuracy, feed_dict=test_feed) # 测试集精度
                print("After %d training step(s) testing accuracy using average model is %g" 
                        % (step, test_acc))
...
def main(argv=None):
    ...
    print(mnist.train.images.shape, mnist.train.labels.shape) # Train sets
    print(mnist.test.images.shape, mnist.test.labels.shape) # test sets
    print(mnist.validation.images.shape, mnist.validation.labels.shape) # validation sets
    train(mnist)
