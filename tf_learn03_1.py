...
def train(mnist):
    ...
    # 第一个卷积层[5, 5, 1, 32]代表:卷积核尺寸为5x5,1个通道,32个卷积核
    # 创建滤波器权值-->加偏置-->卷积-->池化
    W_conv1 = weight_variable([5, 5, 1, 32])  # fliter1
    b_conv1 = bias_variable([32])
    h_conv1 = tf.nn.relu(
        conv2d(x_image, W_conv1) + b_conv1)  # 28x28x1 与32个5x5x1滤波器 --> 28x28x32
    h_pool1 = max_pool_2x2(h_conv1)  # 28x28x32 -->14x14x32

    # 第二层卷积层 卷积核依旧是5x5 通道为32   有64个不同的卷积核
    W_conv2 = weight_variable([5, 5, 32, 64])  # fliter2
    b_conv2 = bias_variable([64])
    h_conv2 = tf.nn.relu(conv2d(h_pool1, W_conv2) +
                         b_conv2)  # 14x14x32 与64个5x5x32滤波器 --> 14x14x64
    ...
    h_fc1 = tf.nn.relu(
        tf.matmul(h_pool2_flat, W_fc1) + b_fc1)  # FC层传播 3136 --> 1024
    ...
    # tf.argmax - 返回vector中的最大值的索引号:
    #   如果vector是一个向量，那就返回一个值
    #   如果是一个矩阵，那就返回一个向量(其每一维都是相对应矩阵行的最大值元素的索引号)
